...
const navLinks = {
  ...
  "Pricing": "/pricing",
  "Membership List": "/listpage",
  "Job Portal": "/job-portal",  // Changed from "JobPortal" to "Job Portal"
  "AI Interview Practice": "/ai-interview",  // Added this line
  "Resume Analyzer": "/resume-analyzer",  // Added this line
...
const NavItem = ({ icon: Icon, text, active, onClick, isSubItem = false, isNestedSubItem = false }) => (
  <li
    onClick={onClick}
    className={`flex items-center space-x-3 rounded-xl cursor-pointer transition-all duration-200 ${active
      ? "bg-gradient-to-r from-blue-500/20 to-indigo-500/20 text-blue-700 font-semibold border border-blue-300/50"
      : "text-slate-700 hover:bg-white/60 hover:shadow-md"
      } ${isNestedSubItem ? "py-2 px-3 ml-4" : isSubItem ? "py-2 px-3 ml-2" : "p-3"}`}
  >
    ...
    if (item.children) {
      return (
        <li key={item.name}>
          <div
            onClick={() => toggleMenu(item.name)}
            className={`flex items-center justify-between space-x-3 rounded-xl cursor-pointer transition-all duration-200 ${openMenus.includes(item.name) ||
              item.children.some((c) => c.name === activeItem || (c.children && c.children.some(nested => nested.name === activeItem)))
              ? "bg-gradient-to-r from-blue-500/20 to-indigo-500/20 text-blue-700 font-semibold shadow-md border border-blue-300/50"
              : "text-slate-700 hover:bg-white/60 hover:shadow-md"
              } ${isSubItem ? "py-2 px-3 ml-2" : "p-3"}`}
          >
            ...
            <ChevronDown
              size={16}
              className={`transition-transform duration-300 ${openMenus.includes(item.name) ? "rotate-180" : "rotate-0"
                }`}
            />
          </div>
